...
for line in $filelines;do
    ...
    outputFile="./output/vds_"$counter"_"$line".json"
    #echo $outputFile
    ...
    yt-dlp --print-json --flat-playlist  $base_url$line"/videos" | jq -s 'sort_by(-.view_count) | .[:100] | .[] | {title: .title, id: .id, views: .view_count, url: .url}' >> $outputFile
